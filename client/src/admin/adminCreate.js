...
export default function AdminCreate() {
    ...
    function validateFields() {
        ...
        if (isNaN(price) || price <= 0) {
            const editedData = { ...formData, ...formData['price'].isValid = false }
            setFormData(editedData)
        } else {
        ...
        if (typeof description === 'number' || description === '') {
            const editedData = { ...formData, ...formData['description'].isValid = false }
            setFormData(editedData)
        } else {
            const editedData = { ...formData, ...formData['description'].isValid = true }
            setFormData(editedData)
        }

        if (typeof image === 'number' || image === '') {
            const editedData = { ...formData, ...formData['image'].isValid = false }
            setFormData(editedData)
        } else {
    ...
    return <>
        ...
        <div className='sectionCreate'>
            ...
            <form onSubmit={onSubmit} className='createForm'>
                <label>name</label>
                <input type='text' name='name' required value={formData['name'].value} onChange={(e) => onChange(e)} />

                <label>price in BGN</label>
                <input type='text' name='price' required value={formData["price"].value} onChange={(e) => onChange(e)} />

                <label>description</label>
                <input type='text' name='description' required value={formData['description'].value} onChange={(e) => onChange(e)} />

                <label>image</label>
                <input type='text' name='image' value={formData['image'].value} required onChange={(e) => onChange(e)} />

                <button type='submit' className='createBtn' disabled={isButtonDisabled()}>Submit</button>
            </form>
        </div>
    </>
}